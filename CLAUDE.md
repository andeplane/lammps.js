...
- `cpp/` — C++ glue code and build tooling:
  ...
  - `cpp/build.py` — CMake/Emscripten build script; clones LAMMPS (tag pinned via `LAMMPS_TAG`) and emits `dist/cpp/lammps.js`.
...
- Keep the Emscripten version in `emsdk_manifest.txt` in sync with CI (`.github/workflows/*.yml`) — the CI cache key hashes that file.
