...
type SceneSnapshot = {
  particles: { count: number; positions: Float32Array };
  bonds: { count: number; first: Float32Array; second: Float32Array };
  box: { matrix: Float32Array; origin: Float32Array };
};
...
export const createScene = (canvas: HTMLCanvasElement): SceneHandles => {
  ...
  const update = (snap: SceneSnapshot) => {
    const pos = snap.particles.positions;
    const count = Math.min(snap.particles.count, MAX_PARTICLES);
    atoms.count = count;
    for (let i = 0; i < count; i++) {
      const idx = i * 3;
    ...
    const bondsBuffer = snap.bonds.count
      ? packBonds(snap.bonds.first, snap.bonds.second)
      : new Float32Array(0);
    bondsGeometry.setAttribute(
      "position",
      new THREE.Float32BufferAttribute(bondsBuffer, 3)
    );
    bondsGeometry.setDrawRange(0, snap.bonds.count * 2);
    if (snap.bonds.count > 0) {
      bondsGeometry.computeBoundingSphere();
    }

    const boxBuffer = buildBox(snap.box.matrix, snap.box.origin);
